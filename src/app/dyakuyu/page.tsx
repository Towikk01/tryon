...
export default async function ThankYouPage({
  ...
  // Фолбек: статичне посилання, якщо reference немає (напр. ручний перехід).
  const fallbackLink = process.env.NEXT_PUBLIC_TELEGRAM_LINK;
    ...
    <main className="theme-blush flex min-h-screen items-center justify-center bg-cream px-5 py-16 text-ink">
      <div className="w-full max-w-md text-center">
        ...
        <p className="mt-4 text-lg leading-relaxed text-ink-soft">
          Оплата отримана. За хвилину нижче з&apos;явиться персональне посилання —
          натисни його, щоб відкрити курс у Telegram. Там на тебе чекають усі
          матеріали та підтримка.
        </p>

        {reference ? (
          <AccessLink reference={reference} />
        ) : (
          fallbackLink && (
            <Link
              href={fallbackLink}
